...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			// 		argValue = decodeURIComponent(pairs[i].substring(pos + 1));
			// 		args[argName] = argValue;
			// 	}
			// 	return args[para] ? args[para] : "";
			// }
		...
		function fuc() {
				...
				for (var i = 0; i < _zb_checkboxs_searchYinsu1.length; i++) {
					if (_zb_checkboxs_searchYinsu1[i].checked) {
						//console.log(_zb_checkboxs_searchYinsu1[i].value);
				...
				for (var j = 0; j < _zb_checkboxs_searchYinsu2.length; j++) {
					if (_zb_checkboxs_searchYinsu2[j].checked) {
						//console.log(_zb_checkboxs_searchYinsu2[j].value);
...
<!-- 	<script src="js/global.js" type="text/javascript" charset="utf-8"></script>
	...
	<script type="text/javascript">
		...
		jQuery(function(){
				...
				_zb_tempHtml += '<div class="col w3">';
					_zb_tempHtml += '<div class="content">';
						_zb_tempHtml += '<fieldset>';
							...
							_zb_tempHtml += '<table style="display:none;">';
							_zb_tempHtml += '<tbody>';
			...
			// 展示和收缩地区服务器列表
			var _zb_css_diplay = 'none',
				_zb_first_small,
				_zb_only_table,
				_zb_ServerAreas = $(_zb_searchYinsu1).find('.col.w3');
			_zb_ServerAreas.on('mouseover', 'div.content', function() {
				_zb_first_small = this.querySelector('small');
				_zb_only_table =  this.querySelector('table');
				_zb_only_table.style.display = 'block';
				_zb_first_small.className = '_zb_icon minus';

			})
			_zb_ServerAreas.on('mouseout', 'div.content', function() {
				_zb_first_small = this.querySelector('small');
				_zb_only_table =  this.querySelector('table');
				_zb_only_table.style.display = 'none';
				_zb_first_small.className = '_zb_icon plus';
			})
